...
class LoginController extends Controller
{
    ...
    public function doAdminLogin(Request $request)
    {
        if ($request->isMethod('get')) {
            ...
            }elseif (Auth::attempt(['email' => $request->email, 'password' => $request->password, 'user_type' => '3'])) {
                /* user type admin and login is successful */
                return redirect('agent/dashboard');
            }elseif (Auth::attempt(['email' => $request->email, 'password' => $request->password, 'user_type' => '4'])) {
                /* user type admin and login is successful */
